...
if __name__ == '__main__':
    ...
    my_table = 'log'
    print('Test SQLiteHandler\n\tdatabase: {}\n\ttable: {}'.format(my_database, my_table))
    logger = logging.getLogger(__name__)
    logger.setLevel(logging.DEBUG)

    sql_handler = SQLiteHandler(database = my_database, table = my_table, attributes_list = DEFAULT_ATTRIBUTES_LIST)
    sql_handler.setLevel(logging.DEBUG)
    formatter = logging.Formatter('%(' + ((')s' + DEFAULT_SEPARATOR + '%(').join(DEFAULT_ATTRIBUTES_LIST)) + ')s')
